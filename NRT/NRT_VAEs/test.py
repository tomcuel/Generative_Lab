...
# ===========================
# Testing VAEs on CIFAR-10
# ===========================
def test_CNN_BaseVAE_cifar10():
    """
    Test the CNN-based VAE on the CIFAR-10 dataset with downsampling to 16x16 and 3 channels for RGB

    This function loads the CIFAR-10 dataset with downsampling and rgb channels,
    initializes a VAE with an CNN architecture,
    trains it for a few epochs, save and load the model,
    and then verify the called methods for reconstruction and sampling
    """
    ...
    # Train VAE for a few epochs
    print("Training VAE...")
    metrics = vae.fit(cifar_loader, epochs=5, verbose=True)
    print()
    print("Training metrics:")
    for epoch, metric in enumerate(metrics):
        print(f"Epoch {epoch+1}: Loss={metric.loss:.4f}, Recon Loss={metric.recon:.4f}, KL Div={metric.kld:.4f}")
    ...
    # Save / Load model 
    print("Saving...")
    vae.save(os.path.join(MNIST_DIR, "cnn_vae_cifar10.pth"))
    cfg.save(os.path.join(MNIST_DIR, "cnn_vae_config_cifar10.json"))
    print("Model saved to cnn_vae_cifar10.pth and configuration saved to cnn_vae_config_cifar10.json")
    print()

    print("Loading...")
    cfg = VAEConfig()
    cfg.load(os.path.join(MNIST_DIR, "cnn_vae_config_cifar10.json"))
    vae = BaseVAE(cfg, device=device)
    vae.load(os.path.join(MNIST_DIR, "cnn_vae_cifar10.pth"))
    print("Model loaded from cnn_vae_cifar10.pth with configuration from cnn_vae_config_cifar10.json")
    print()

    # Test reconstructions and sampling
    x_test, _ = next(iter(cifar_loader))
    x_test = x_test.to(device)
    print("Test batch shape:", x_test.shape)  # Should be (128, 256)
    ...
    vae.plot_image_reconstruction(x_test, n=10, save_path=os.path.join(MNIST_DIR, "mnist_reconstructions_cnn_vae_cifar10.png"))
    vae.plot_image_samples(n=20, n_rows=4, save_path=os.path.join(MNIST_DIR, "mnist_samples_cnn_vae_cifar10.png"))
...
if __name__ == "__main__":
    ...
    print_section("Testing CNN VAE architectures on CIFAR-10 dataset")
