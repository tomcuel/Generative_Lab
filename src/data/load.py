...
def load_cifar10(
    batch_size: int = 128,
    downsample: tuple[int, int] | None = (32, 32),
    train: bool = True,
    grayscale: bool = False
) -> DataLoader:
    """
    Load the CIFAR-10 dataset with optional downsampling and grayscale conversion
    ...
    batch_size: int
        Number of samples per batch to load
    downsample: tuple[int, int] | None
        If None, images will be flattened to (C*H*W,)
        If a tuple (new_h, new_w), images will be resized to (new_h, new_w) and then flattened to (C*new_h*new_w,)
    train: bool
        If True, load the training set; otherwise, load the test set
    grayscale: bool
