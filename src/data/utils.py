...
def plot_images(
    ...
    def _reshape_for_display(img: np.ndarray) -> np.ndarray:
        if img.ndim == 1:
    ...
    for i in range(n):
        img = _reshape_for_display(images[i])
        # Handle channel-first images (C, H, W) coming from PyTorch tensors
        if img.ndim == 3 and img.shape[0] in (1, 3):
            img = np.transpose(img, (1, 2, 0))
            if img.shape[2] == 1:
                img = img[:, :, 0]
...
def merge_images(
    ...
    -----------
    images: torch.Tensor or np.ndarray
        Batch of images to merge, shape (N, H, W) or (N, H, W, C) or (N, H*W)
    n_row: int or None
    ...
    # Handle 1D flattened square images
    if images.ndim == 2 and len(images[0].shape) == 1:
